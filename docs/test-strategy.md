...
### 5.5 CI と pre-push hook の役割分担 (Issue #80 / PR #85 の経緯)

CI 高速化のため Pest を `--parallel` で実行 (Issue #80) すると、xdebug の compound branch カウントが worker process 間で **実行順 / 分配次第で変動** し、AdminApiExceptionRenderer / UpdateCategoryUseCase / ConferenceController 等で閾値割れが頻発した:

| 環境 | AdminApiExceptionRenderer (covered/total) |
|---|---|
| Serial (PR #14 当時) | 39/49 = 79.59% |
| Serial (PR #85 計測時) | 42/54 = 77.78% |
| Parallel ローカル | 44/57 = 77.19% |
| **Parallel CI** | **42/57 = 73.68%** ⚠️ |

`total` が 49 → 54 → 57 と増え `covered` も 1〜2 個変動するのは xdebug + ParaTest の implementation noise (= テスト品質ではない)。本層の閾値だけでなく Application/Controller 層にも波及するため、閾値を局所的に下げる対応では設計ポリシー (Application 100% 必須等) が崩れる。

**採用方針 (Issue #80 D 案 + PR #85)**: CI ↔ pre-push hook の役割分担で解決:

- **CI**: `pest --parallel --no-coverage` で速度優先 (テスト pass のみ判定、coverage 計測なし)。Dependabot や GitHub Web UI 経由の編集でも機能リグレッション検出は維持される
- **pre-push hook (.githooks/pre-push)**: `make api-test-coverage` で **serial 実行 + xdebug coverage** を計測し `make api-coverage-check` で層別 C1 ゲート判定。ローカルで coverage が落ちる push は弾く

これにより CI 全体の wall clock を ~317s → ~30s に短縮しつつ (約 90% 削減)、層別 C1 設計と PR #14 当時の 75% 閾値を維持できる。
...
```sh
make api-phpstan         # PHPStan level max (~10-30s)
...
| Issue #15 | 本ドキュメントの整備 |
| Issue #13 | PHPStan 導入 (admin-api) とレベル最大の commit/push ゲート化 |

---

## 10. PHPStan (静的型解析) との関係

Issue #13 で導入した PHPStan level max ゲートは、本ドキュメントが扱う「テスト
で振る舞いを担保する」のと並列に「型レベルで矛盾を弾く」役割を担う。両者は
独立したゲートで、`pre-push` で順次実行する。

### 適用範囲

- **`app/` 配下**: level max でゼロエラー必須。新規違反は pre-push で遮断
- **`tests/` 配下**: Pest クロージャの `$this` 推論不足、Mockery fluent API の
  union 型解決困難など、ツール構造由来のノイズ起源 identifier を限定 ignore
  (詳細は `apps/admin-api/phpstan.neon` の `ignoreErrors` 参照)

### テスト記法との関係

PHPStan が tests/ で多くを ignore している以上、テストコードの「型として正しい
こと」は人間レビューで担保する。具体的には:

- Mockery のメソッドチェーンは `andReturn(/* 期待型 */)` の型を意識する (PHPStan
  が見えなくても返り値の構造は呼出側 (UseCase 等) と整合させる)
- `$response->json('foo.bar')` で取り出した値を assertion に渡す時は `expect()`
  か Laravel `assertJsonPath()` を使い、生 mixed のまま ` === ` 比較しない
...
## 11. TODO (本ドキュメントの将来追記)

- [ ] フロントエンド (Vitest + RTL) の閾値・運用 — `apps/frontend` 着手時
- [ ] CI 環境 (GitHub Actions 等) でのゲート — CI 導入時
- [ ] Pest / Mockery の PHPStan 完全対応が来たら tests/ の ignoreErrors 削除を検討
