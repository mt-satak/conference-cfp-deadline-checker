...
it('PublicConferencePresenter::toArray は公開フロントが読む既存フィールドを返す (= 互換性維持)', function () {
    // Given: フル Conference
    $conf = makePublicPresenterFullConference();
...
it('PublicConferencePresenter::toArray は trackName / description を出力しない (Issue #204)', function () {
    // Given: trackName と description に値が入った Conference
    $conf = makePublicPresenterFullConference();
    expect($conf->trackName)->toBe('Track A');
    expect($conf->description)->toBe('PHP の祭典');

    // When
    $array = PublicConferencePresenter::toArray($conf);

    // Then: 公開出力には含まれない (= フロント表示不要のため Issue #204 で撤去)
    expect($array)->not->toHaveKey('trackName');
