...
it('?status=draft で UseCase に [Draft] フィルタが渡る (Phase 0.5 / Issue #165)', function () {
    // Given: UseCase は [Draft] 配列フィルタで呼ばれることを期待
    $useCase = Mockery::mock(ListConferencesUseCase::class);
...
it('?status=active で UseCase に [Draft, Published] フィルタが渡る (Issue #165)', function () {
    ...
    // When
    $response = $this->getJson('/admin/api/conferences?status=active');
...
it('?status に未知値があってもフィルタなしで処理する (fail-soft)', function () {
    ...
    // When
    $response = $this->getJson('/admin/api/conferences?status=unknown-status');
